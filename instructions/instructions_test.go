...
func fixtureRoot(t *testing.T) string {
	t.Helper()
	abs, err := filepath.Abs(filepath.Join("..", "testdata", "instructions"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRender(t *testing.T) {
	...
	golden := filepath.Join("..", "testdata", "golden", "instructions", "render.txt")
	if *update {
	...
	if err != nil {
		t.Fatalf("%v (run go test ./instructions -update)", err)
	}
	if got != string(want) {
		t.Errorf("Render() =\n%s\nwant\n%s", got, want)
	}
