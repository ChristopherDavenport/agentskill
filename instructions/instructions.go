// Package instructions finds and renders the instruction files of the
...
var ErrTooLarge = errors.New("instructions: file exceeds size limit")
...
func Chain(path string, opts Options) (Result, error) {
	...
	if opts.Root != "" {
		...
		if err != nil {
			return Result{}, fmt.Errorf("instructions: %w", err)
		}
		if start != root && !strings.HasPrefix(start, root+string(filepath.Separator)) {
			return Result{}, fmt.Errorf("instructions: %s is outside root %s", path, root)
		}
	}
	var dirs []string
...
func stat(path string) (os.FileInfo, bool, error) {
	...
	if err != nil {
		...
		return nil, false, fmt.Errorf("instructions: %w", err)
	}
	if info.IsDir() {
		return nil, false, nil
	}
